...
exports.aiAssistantHttp = exports.aiAssistant = void 0;
...
exports.aiAssistant = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'L\'utente deve essere autenticato per utilizzare questa funzione');
        }
        // Ottieni i dati dalla richiesta
        const { message, conversation = [] } = data;
        if (!message || typeof message !== 'string') {
            throw new functions.https.HttpsError('invalid-argument', 'È necessario fornire un messaggio valido');
        }
        ...
        if (!OPENAI_API_KEY) {
            throw new functions.https.HttpsError('failed-precondition', 'Chiave API OpenAI non configurata');
        }
        ...
        if (error.response && error.response.data) {
            throw new functions.https.HttpsError('internal', ((_a = error.response.data.error) === null || _a === void 0 ? void 0 : _a.message) || 'Errore nel servizio OpenAI', { details: error.response.data });
        }
        throw new functions.https.HttpsError('internal', error.message || 'Si è verificato un errore sconosciuto', { originalError: error.toString() });
    }
});
/**
 ...
 */
exports.aiAssistantHttp = functions.https.onRequest(async (request, response) => {
    var _a;
    // Abilita CORS
    ...
    try {
        ...
        if (error.response) {
            ...
            response.status(error.response.status || 500).send({
                error: 'Errore OpenAI',
                message: ((_a = error.response.data.error) === null || _a === void 0 ? void 0 : _a.message) || 'Errore nel servizio OpenAI'
            });
            return;
        }
        // Errore generico
